...
class machine(Machine, NYUDepthModelDefs):
    ...
    def define_scale3_onestack(self, stack_type, p_1_drop, p_1_mean, crop_size):
    ...
    def define_normals_cost(self, pred, y0, m0):
        bsize = self.bsize
        npix = int(np.prod(test_shape(y0)[2:]))
        y0_target = y0.reshape((self.bsize, 3, npix))
        y0_mask = m0.reshape((self.bsize, npix))[:,xx,:]
        pred = pred.reshape((self.bsize, 3, npix))
        # cosine distance of normalized vectors
        error = -T.sum(pred * y0_target * y0_mask) / T.sum(y0_mask)
        # scale relative normals cost
        cost = 10 * error
        return (error, cost)
